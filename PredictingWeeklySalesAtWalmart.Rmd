...
knitr::opts_chunk$set(cache=TRUE, out.width='400px' , dpi=300 )  
...
The **Standard Deviation** for <code>Weekly_Sales</code> is ```r round( sd(train$Weekly_Sales) , 2)```. The **mean** is ```r round( mean(train$Weekly_Sales) , 2 )``` and **median** is ```r median(train$Weekly_Sales)```. The mean and the median are very far apart, indicating that the data is skewed - in this case, extremely **right-skewed**. The following histogram depicts this relationship - where you can clearly observe the long tail towards the right making it extremely right-skewed.
...
#### 3.1.1.2 Dealing with Negative and Zero Weekly Sales
...
```{r dealingWithNegative0WeeklySales}
...
* The data set represents a paltry ```r paste0( round( nrow(train0)/nrow( train )*100 , 1) , "%" )``` of the full dataset - it has ```r nrow( train0 )``` observations
...
```{r calculatingDetailedSummaryStatisticsOfWeekly_Sales}
...
```{r calculatingDetailedSummaryStatisticsOfLog_Weekly_Sales}
...
```{r descriptionOfDefailedSummaryStatistics}
...
```{r printingDetailedSummaryStatisticsTable}
...
```{r StoreSizeSummaryStatisticsBasedOnType}
...
```{r calculateMeanCPIAcrossAllStores}
...
```{r CPIMissingHeatMap, warning=FALSE}
...
```{r CPIMeanMinMaxRangeCalculation}
